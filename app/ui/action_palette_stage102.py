...
def _backend_client() -> httpx.AsyncClient:
    """Keep one loopback connection pool for interactive chat turns."""
...
async def _http_probe(url: str, *, json_expected: bool = False, timeout: float = 4.0):
    try:
        async with httpx.AsyncClient(timeout=timeout, trust_env=False, headers=auth_headers()) as client:
