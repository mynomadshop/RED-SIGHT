...
# IMPORTANT: never stage inside SourceRoot. A robocopy source -> child destination is recursive
# and can return fatal exit code 16. Keep staging in a sibling temp directory instead.
...
try {
  ...
  $pyExtract = Join-Path $env:TEMP "redsight-python-extract-$buildId"
  ...
  if ($buildPython) {
    $hostPython = 'py.exe'; $hostPythonArgs = @('-3.12')
  } else {
    $pythonCmd = Get-Command python.exe -ErrorAction SilentlyContinue
    if ($pythonCmd) {
      $pyHost = (& $pythonCmd.Source --version 2>&1 | Out-String).Trim()
      if ($pyHost -notmatch '^Python 3\.12') { $pythonCmd = $null }
    }
    if (-not $pythonCmd) {
      $winget = Get-Command winget.exe -ErrorAction SilentlyContinue
      if (-not $winget) { Fail 'Python 3.12 is required on the build host to populate bundled dependencies.' }
...
"@
  ...
  # ISCC /? is help output and its process result is not a meaningful build-health check.
  # Only compilation of the real .iss script determines success.
  ...
  $manifest = [ordered]@{
    product='RedSight'; distribution='Windows v6'; sourceVersion='11.5.5'; installerVersion=$AppVersion;
    builtAt=(Get-Date).ToUniversalTime().ToString('o'); architecture='x64'; installer='RedSight-Setup-v6.exe';
    size_bytes=$size; sha256=$hash; wsl='WSL2 enabled/provisioned by resumable bootstrapper';
    docker='Docker Desktop WSL2 backend provisioned by resumable bootstrapper'; offline_bundle=[bool]$BundleOfflineDependencies;
    python='3.12.10 private runtime with packaged dependencies'; playwright='Chromium bundled';
