...
if (-not (Test-Path $inno)) {
  ...
  if ($winget) { & winget install --id JRSoftware.InnoSetup.7 -e --silent --accept-source-agreements --accept-package-agreements | Out-Host }
  if (-not (Test-Path $inno)) { Fail 'Inno Setup 7 x64 ISCC.exe is required.' }
}
Remove-Item $OutputRoot -Recurse -Force -ErrorAction SilentlyContinue
$stage = Join-Path $OutputRoot 'payload'; $offline = Join-Path $stage 'offline'; $setupDir = Join-Path $OutputRoot 'installer'; $artifacts = Join-Path $OutputRoot 'artifacts'
...
$tools = Join-Path $stage 'tools'; New-Item -ItemType Directory -Force -Path $tools | Out-Null
$pyPkg = Join-Path $env:TEMP 'redsight-python-3.12.10.nupkg'; $pyExtract = Join-Path $env:TEMP 'redsight-python-extract'
