...
if (-not $inno) {
  ...
  if (-not $winget) { Fail 'Inno Setup 7 is missing and winget.exe is unavailable. Install Inno Setup 7 x64 and rerun.' }
  Write-Host 'Inno Setup 7 not found. Installing the official 64-bit package...'
  & winget install --id JRSoftware.InnoSetup.7 -e -s winget --silent --accept-source-agreements --accept-package-agreements | Out-Host
  $inno = Find-InnoCompiler
}
if (-not $inno) { Fail 'Inno Setup 7 installation completed but ISCC.exe could not be located. Check Program Files\Inno Setup 7 and Program Files (x86)\Inno Setup 7.' }
...
$required = @(
  ...
  'redsight_actions\gateway.py',
  'scripts\windows\RedSight-Setup-Resume.ps1',
  'scripts\windows\RedSight-PostInstall-Health.ps1',
  'scripts\windows\Start-RedSight.ps1',
  'assets\redsight.ico'
)
foreach ($r in $required) { if (-not (Test-Path (Join-Path $stage $r))) { Fail "Required runtime file missing: $r" } }

# Build a self-contained Python runtime payload and populate its site-packages.
$tools = Join-Path $stage 'tools'
$site = Join-Path $tools 'Lib\site-packages'
New-Item -ItemType Directory -Force -Path $tools, $site | Out-Null
$pyPkg = Join-Path $env:TEMP 'redsight-python-3.12.10.nupkg'
$pyExtract = Join-Path $env:TEMP 'redsight-python-extract'
...
$pyExe = Join-Path $tools 'python.exe'
if (-not (Test-Path $pyExe)) { Fail 'Bundled Python runtime was not produced.' }
$pv = & $pyExe --version 2>&1 | Out-String
if ($pv -notmatch 'Python 3\.12') { Fail "Unexpected bundled Python version: $pv" }

$buildPython = Get-Command py.exe -ErrorAction SilentlyContinue
if ($buildPython) {
  $pyHost = (& py.exe -3.12 --version 2>&1 | Out-String).Trim()
  if ($pyHost -notmatch '^Python 3\.12') { $buildPython = $null }
}
if (-not $buildPython) {
  $pythonCmd = Get-Command python.exe -ErrorAction SilentlyContinue
  if ($pythonCmd) {
    $pyHost = (& python.exe --version 2>&1 | Out-String).Trim()
    if ($pyHost -notmatch '^Python 3\.12') { $pythonCmd = $null }
  }
  if (-not $pythonCmd) {
    $winget = Get-Command winget.exe -ErrorAction SilentlyContinue
    if (-not $winget) { Fail 'Python 3.12 is required on the build host to populate the bundled runtime dependencies.' }
    & winget install --id Python.Python.3.12 -e --silent --accept-source-agreements --accept-package-agreements | Out-Host
    $pythonCmd = Get-Command python.exe -ErrorAction SilentlyContinue
  }
}
if ($buildPython) { $hostPython = 'py.exe'; $hostPythonArgs = @('-3.12') } else { $hostPython = $pythonCmd.Source; $hostPythonArgs = @() }
...
try {
  & $hostPython @hostPythonArgs -m playwright install chromium | Out-Host
  if ($LASTEXITCODE -ne 0) { Fail "Playwright Chromium installation failed with exit code $LASTEXITCODE" }
} finally {
...
if ($BundleOfflineDependencies) {
  ...
  $a = $rel.assets | Where-Object { $_.name -match '\.x64\.msi$' } | Select-Object -First 1
...
# Inno Setup paths are emitted literally; Windows backslashes are not doubled here.
$sourceRootIss = $stage
$outputRootIss = $artifacts
$issText = @"
#define AppName "RedSight"
#define AppVersion "$AppVersion"
#define SourceRoot "$sourceRootIss"
#define OutputRoot "$outputRootIss"
[Setup]
...
& $inno /Q /O+ $iss | Out-Host
if ($LASTEXITCODE -ne 0) { Fail "Inno Setup compilation failed with exit code $LASTEXITCODE. See $iss and the compiler output above." }

$exe = Join-Path $artifacts 'RedSight-Setup-v6.exe'
if (-not (Test-Path $exe)) { Fail 'Installer EXE was not produced.' }
$hash = (Get-FileHash -Algorithm SHA256 $exe).Hash.ToLowerInvariant()
$size = (Get-Item $exe).Length
$manifest = [ordered]@{
  product='RedSight';
  distribution='Windows v6';
  sourceVersion='11.5.5';
  ...
  size_bytes=$size;
  sha256=$hash;
  wsl='WSL2 enabled/provisioned by resumable bootstrapper';
  docker='Docker Desktop WSL2 backend provisioned by resumable bootstrapper';
  offline_bundle=[bool]$BundleOfflineDependencies;
  python='3.12.10 private runtime with packaged dependencies';
  playwright='Chromium bundled';
  notes='Windows x64 distribution. WSL2 requires hardware virtualization. Online mode downloads current Microsoft WSL and Docker Desktop components.'
}
...
$readme = "RedSight Windows v6 distribution`r`n`r`nInstaller: RedSight-Setup-v6.exe`r`nSHA256: $hash`r`nArchitecture: Windows x64`r`n`r`nThe installer enables WSL2, restarts only when required, resumes automatically after logon, provisions Docker Desktop for WSL2, packages its Python runtime and runtime dependencies, and runs a post-install health gate. Online mode downloads current Microsoft WSL and Docker components. Use -BundleOfflineDependencies for a larger offline-capable build.`r`n"
